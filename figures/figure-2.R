...
keep_targets <- bind_rows(sim_results$evaluations,
...
avg_sensitivity <- est |>
  ...
  mutate(size = factor(size, levels = c("large", "medium", "small"),
                       labels = c( "Large (m = 216, n = 72)",
                                   "Medium (m = 108, n = 36)",

                                   "Small (m = 54, n = 18)")),
         term = factor(term, levels = c("conditionB",
                                        "timet3:conditionB"),
                       labels = c("&beta;<sub>1</sub> group differences",
                                  "&beta;<sub>5</sub> group&times;time")),
         disp_scenario = factor(disp_scenario, levels = c("s2", "s1"),
                                labels = c("Low variability",
                                           "High variability")),
         model_type = if_else(model %in% c("m1", "m2"), "nb",
...
p1 <- est |>
  ...
  geom_point(data = avg_sensitivity,
             aes(model, m, group = NULL, fill = model, shape = model,
                 alpha = size),
  ...
  labs(y = "Sensitivity (% of true effects detected)",
       fill = "Model",
       shape = "Model",
       color = "Sample size") +
...
p2 <- est |>
  ...
  labs(y = "Error rate (% of true negative effects)",
       fill = "Model",
       shape = "Model",
       color = "Sample size") +
  scale_fill_manual(values = c(colors[1], colors[2], colors[4], colors[1], colors[2])) +
...
figure2 <- plot_grid(
  ...
  plot_grid(p3, ggdraw(legend),
                    ...
                    ncol = 1),
  rel_widths = c(1, 1)) +
  annotate("text", x = c(0.02, 0.02, 0.53),
           y = c(0.98, 0.49, 0.98), label = c("A","C", "B"))




## Save RDS ##
saveRDS(figure2, "figures/figure-2.RDS")
