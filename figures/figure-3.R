...
p1 <- n_estimates |>
  ...
  geom_text(aes(model, non_est_prop + 1.1, label = non_est),
            size = 2.5) +
...
p1b <- data.frame(pred) |>
  ...
  scale_x_continuous(limits = c(1040, 1080),
                     expand = c(0,0)) +

  geom_segment(data = delta_aic,
               aes(y = xmin, yend = xmax,
                   x = ycoord, xend = ycoord)) +
  geom_segment(data = delta_aic,
               aes(y = xmin, yend = xmin,
                   x = ycoord, xend = ycoord-1)) +
  geom_segment(data = delta_aic,
               aes(y = xmax, yend = xmax,
                   x = ycoord, xend = ycoord-1)) +



  geom_text(data = delta_aic,
               aes(y = xmax - (xmax - xmin)/2 + 0.5,
                   x = ycoord + 0.5,
                   hjust = 0,
                   label = prop),
            size = 2.5) +
...
p2 <- upset(sig_genes_df, intersect = setdiff(names(sig_genes_df), "target"),
      name = "",
      height_ratio = 1.2,
      width_ratio = 0.2,
      stripes = c(alpha(colors[4],0.2),
                  alpha(colors[1],0.2),
                  alpha(colors[1],0.2),
                  alpha(colors[2],0.2),
                  alpha(colors[2],0.2)),
      ...
      base_annotations = list(
        'Intersection size'= (
          intersection_size(text_mapping=aes(
...
anno_df <-  stat |>
    ...
    filter(distance.neg == min(distance.neg) |
...
gsea_plot_up <-  bind_rows(
  ...
  ggplot(aes(xrelative, runningScore, color = model)) + geom_line() +
  theme_classic(base_size = 8) +
...
figure3 <-  plot_grid(
   plot_grid(NULL, p1, p1b, NULL,
            ncol = 4, rel_widths = c(0.05,0.7, 1, 0.05)),
   NULL,
   p2,
   p3,
   NULL,
